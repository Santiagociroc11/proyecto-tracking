...
export function AdDetailsModal({ isOpen, onClose, ad }: AdDetailsModalProps) {
  ...
  const fetchAdPreview = useCallback(async () => {
    ...
    try {
      ...
      const version = "v22.0";
      const url = `${baseUrl}/${version}/${ad.ad_id}/previews`;
  ...
  useEffect(() => {
    if (isOpen && activeTab === "preview" && !adPreview) {
      fetchAdPreview();
    }
  }, [isOpen, activeTab, adPreview, fetchAdPreview]);
  ...
  const calculateTrends = () => {
    ...
    const roasLast7 = last7DaysSpend > 0 ? (last7DaysSales * 4.39) / last7DaysSpend : 0;
    const roasPrevious7 = previous7DaysSpend > 0 ? (previous7DaysSales * 4.39) / previous7DaysSpend : 0;
        ...
        <div className="sticky top-0 bg-white p-4 border-b border-gray-200 flex justify-between items-center z-10">
          <div>
            ...
            <div className="flex items-center mb-1">
              <Layers className="h-4 w-4 text-indigo-500 mr-1" />
              <span className="text-sm font-medium text-indigo-600">{ad.adset_name}</span>
            </div>
              ...
              <div className="p-4">
                {loadingPreview ? (
                  ...
                  <div className="h-96 flex items-center justify-center text-gray-500">
                    Haz clic en "Vista previa" para ver el anuncio
                  </div>
                )}
              </div>
            </div>
